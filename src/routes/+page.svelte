<script lang="ts">
    ...
    function valueInDictValues(dict: { [key: string]: string | number }, searchValue: string): boolean {
        for (const key in dict) {
            if (typeof dict[key] === 'string' && dict[key].toLowerCase().includes(searchValue.toLowerCase())) {
                return true;
            }
        }
        return false;
    ...
    <AppBar>
        ...
        {#snippet trail()}
            <button
                    ...
                    onclick={() => {
					let tableHtml = "<table>";
					tableHtml += "<thead><tr><th>Vorname</th><th>Nachname</th><th>Straße</th><th>Hausnummer</th><th>Postleitzahl</th><th>Ort</th><th>Partner</th><th>Kinder</th><th>E-Mail</th><th>Festnetz</th><th>Handynummer</th><th>Geburtstag</th></tr></thead>";
					tableHtml += "<tbody>";
					for (const item of contacts) {
  				  tableHtml += `<tr><td>${item.FirstName ? item.FirstName : "-"}</td><td>${item.LastName ? item.LastName : "-"}</td><td>${item.Street ? item.Street : "-"}</td><td>${item.HouseNumber ? item.HouseNumber : "-"}</td><td>${item.ZipCode ? item.ZipCode : "-"}</td><td>${item.City ? item.City : "-"}</td><td>${item.Partner ? item.Partner : "-"}</td><td>${item.Children ? item.Children : "-"}</td><td>${item.Email ? item.Email : "-"}</td><td>${item.Landline ? item.Landline : "-"}</td><td>${item.MobilePhone ? item.MobilePhone : "-"}</td><td>${item.Birthday ? item.Birthday : "-"}</td></tr>`;
  				}
...
<div class="flex flex-wrap gap-4 justify-center items-center pt-[74px]">
    {#each contacts as contact, id (contact.ID)}
        {#if valueInDictValues(contact, searchTerm)}
            <div
                ...
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Festnetz</div>
                    <div class="text-base">{contact.Landline ? contact.Landline : "-"}</div>
                </div>
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Handynummer</div>
                    <div class="text-base">{contact.MobilePhone ? contact.MobilePhone : "-"}</div>
                </div>
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Geburtstag</div>
                    <div class="text-base">{contact.Birthday ? contact.Birthday : "-"}</div>
                </div>
    ...
    {#if showEdit}
        <div class="fixed inset-0 flex items-center justify-center z-10">
            <div class="relative card preset-outlined-primary-500 bg-white w-full max-w-lg p-4 m-3 text-center grid grid-cols-3 gap-4">
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Vorname</div>
                ...
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Festnetz</div>
                    <input class="input" type="text" placeholder="Festnetz"
                           bind:value={contacts[editContact].Landline}/>
                </div>
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Handynummer</div>
                    <input class="input" type="text" placeholder="Handynummer"
                           bind:value={contacts[editContact].MobilePhone}/>
                </div>
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Geburtstag</div>
                    <input class="input" type="text" placeholder="Geburtstag"
                           bind:value={contacts[editContact].Birthday}/>
                </div>
    ...
    {#if showNew}
        <div class="fixed inset-0 flex items-center justify-center z-10">
            <div
                    class="relative card preset-outlined-primary-500 bg-white w-full max-w-lg p-4 m-3 text-center grid grid-cols-3 gap-4">
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Vorname</div>
                ...
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Festnetz</div>
                    <input class="input" type="text" placeholder="Festnetz" bind:value={newContact.Landline}/>
                </div>
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Handynummer</div>
                    <input class="input" type="text" placeholder="Handynummer" bind:value={newContact.MobilePhone}/>
                </div>
                <div class="relative">
                    <div class="text-[0.6rem] text-gray-500">Geburtstag</div>
                    <input class="input" type="text" placeholder="Geburtstag" bind:value={newContact.Birthday}/>
                </div>
