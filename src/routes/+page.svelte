<script lang="ts">
    ...
    let contacts: { [key: string]: string }[] = $state([
        {
            FirstName: 'Max',
            LastName: 'Mustermann',
            Street: 'Musterallee',
    ...
    function valueInDictValues(dict: { [key: string]: string }, searchValue: string): boolean {
        for (const key in dict) {
            if (dict[key].includes(searchValue)) {
                return true;
            }
        }
        return false;
